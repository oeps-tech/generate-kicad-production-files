...
namespace Oeps.KicadProductionFiles.Core.Updates;
...
{
    public const string ExecutableName = "Oeps.KicadProductionFiles.App.exe";
    ...
    private static void ValidateManifest(string directory, SemanticVersion expectedVersion, int supportedRuntimeMajor)
    {
        ...
        foreach (var file in new[] { ExecutableName, "Oeps.KicadProductionFiles.App.dll", "Oeps.KicadProductionFiles.Core.dll", "Oeps.KicadProductionFiles.App.deps.json", "Oeps.KicadProductionFiles.App.runtimeconfig.json", "appsettings.json" })
        {
        ...
        using var runtime = JsonDocument.Parse(File.ReadAllText(Path.Combine(appDirectory, "Oeps.KicadProductionFiles.App.runtimeconfig.json")));
